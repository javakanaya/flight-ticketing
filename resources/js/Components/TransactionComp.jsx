...
const TransactionComp = ({ contactDetailOpen = true, travDetailOpen = true, passengerCount }) => {
    const generateTravellerForms = () => {
        const forms = [];
        for (let i = 1; i <= passengerCount; i++) {
            forms.push(
                <div key={i} className="w-full p-3 font-medium shadow-lg border border-slate-300">
                    <Traveller_Detail title={`Adult ${i}`} open={travDetailOpen} />
                </div>
            );
        }
        return forms;
